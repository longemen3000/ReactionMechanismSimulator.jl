...
@testset "Test Reactors" begin
    ...
    @testset "Test liquid phase Parametrized T Constant V reactor jacobian" begin
        ...
        initialconds = Dict(["ts" => [0.0, 600.0, 1200.0], "T" => [450.0, 490.0, 500.0], "V" => 1.0e-6 * 1e6, "octane" => 6.154e-3 * 1e6, "oxygen" => 4.953e-6 * 1e6])
    ...
    @testset "Parametrized T and P reactor jacobian" begin
        initialconds = Dict(["T" => [1000.0, 1400.0, 1500.0], "ts" => [0.0, 100.0, 200.0], "P" => [1.0e5, 1.8e5, 2.0e5], "H2" => 0.67, "O2" => 0.33]) #Set simulation Initial Temp/Pressure and Volume (function/array)
    ...
    @testset "Parametrized volume adiabatic reactor jacobian" begin
        initialconds = Dict(["T" => 1000.0, "ts" => [0.0, 100.0, 200.0], "V" => [0.01, 0.05, 0.1], "H2" => 0.67, "O2" => 0.33]) #Set simulation Initial Temp/Pressure and Volume (function/array)
    ...
    @testset "Parametrized pressure adiabatic reactor jacobian" begin
        initialconds = Dict(["T" => 1000.0, "ts" => [0.0, 100.0, 200.0], "P" => [2.0e5, 3.0e5, 5.0e5], "H2" => 0.67, "O2" => 0.33])
